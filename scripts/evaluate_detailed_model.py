"""
Evaluate the detailed WTA feature model with a chronological split.

Default split requested for the detailed model:
- train hyperparameter candidates on 2010-2021
- tune on 2022-2023
- hold out 2024-2025 for final testing

When --write-model is used, the saved model is trained on 2010-2023 only.
The 2024-2025 test rows are never used for fitting.
"""
...
def fit(rows: list[dict], c_value: float) -> tuple[StandardScaler, LogisticRegression]:
    ...
    model = LogisticRegression(C=c_value, solver="lbfgs", max_iter=8000)
...
def baseline_metrics(rows: list[dict]) -> dict[str, dict[str, float]]:
    return {
        "overallElo": metrics(rows, np.array([elo_probability(row, "overallEloAdvantage") for row in rows])),
        "matchSurfaceElo": metrics(rows, np.array([elo_probability(row, "matchSurfaceEloAdvantage") for row in rows])),
...
def choose_c(train_rows: list[dict], validation_rows: list[dict], c_values: list[float]) -> tuple[float, list[dict]]:
    ...
    for c_value in c_values:
        ...
        result = metrics(validation_rows, predict(validation_rows, scaler, model))
        result["c"] = c_value
...
def main() -> int:
    ...
    parser.add_argument("--report-output", default="data/model_detailed_evaluation.json")
    parser.add_argument("--model-output", default="data/model_detailed.json")
    parser.add_argument("--train-end-year", type=int, default=2021)
    parser.add_argument("--validation-start-year", type=int, default=2022)
    ...
    parser.add_argument("--c-values", default="0.003,0.01,0.03,0.1,0.3,1,3,10,30")
    ...
    report = {
        "split": {
            ...
            "fitForTestYears": f"2010-{args.test_start_year - 1}",
            "fitForTestRows": len(train_until_test),
            "testYears": f"{args.test_start_year}-2025",
            "testRows": len(test),
        },
        "selectedC": best_c,
    ...
    if args.write_model:
        Path(args.model_output).write_text(
            json.dumps(model_payload(scaler, model, best_c, train_until_test, report), indent=2),
            encoding="utf-8",
        )
